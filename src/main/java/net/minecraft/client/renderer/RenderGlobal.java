...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    private List<ContainerLocalRenderInformation> renderInfosEntitiesShadow = new ArrayList<>(1024);
    ...
    private void renderSky(WorldRenderer worldRendererIn, float posY, boolean reverseX) {
        ...
        for (int i = -384; i <= 384; i += 64) {
            for (int j = -384; j <= 384; j += 64) {
                ...
                worldRendererIn.pos(f, posY, j).endVertex();
                worldRendererIn.pos(f1, posY, j).endVertex();
                worldRendererIn.pos(f1, posY, j + 64).endVertex();
                worldRendererIn.pos(f, posY, j + 64).endVertex();
            }
        }
    }
    ...
    public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            while (iterator1.hasNext()) {
                RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation = iterator1.next();
            ...
            synchronized (this.setTileEntities) {
                for (Object tileentity1 : this.setTileEntities) {
                    if (flag1) {
            ...
            for (Object destroyblockprogress : this.damagedBlocks.values()) {
                ...
                if (flag1) {
                    ...
                    if (tileentity2 != null && Reflector.callBoolean(tileentity2, Reflector.ForgeTileEntity_shouldRenderInPass, i) && Reflector.callBoolean(tileentity2, Reflector.ForgeTileEntity_canRenderBreaking, new Object[0])) {
    ...
    public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty && !Shaders.isShadowPass) {
            ...
            while (!deque.isEmpty()) {
                RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation1 = deque.poll();
            ...
            for (int j1 = -3; j1 <= 4; ++j1) {
                for (int l = -3; l <= 4; ++l) {
                    ...
                    if (f3 > -5.0F) {
                        ...
                        worldrenderer.pos((double) (f24 + 0.0F), (f3 + 0.0F), (f25 + 0.0F)).tex(((f22 + 0.0F) * 0.00390625F + f17), ((f23 + 0.0F) * 0.00390625F + f18)).color(f10, f11, f12, 0.8F).normal(0.0F, -1.0F, 0.0F).endVertex();
                    }
    ...
    public void spawnParticle(int particleID, boolean ignoreRange, final double xCoord, final double yCoord, final double zCoord, double xOffset, double yOffset, double zOffset, int... p_180442_15_) {
        try {
            ...
            crashreportcategory.addCrashSectionCallable("Position", (Callable<String>) () -> CrashReportCategory.getCoordinateInfo(xCoord, yCoord, zCoord));
